...
export default function Contact(props) {
    ...
    const styleInp = "py-3 font-josefin md:text-xl font-medium text-[#0082E0] tracking-[0.1em] outline-none border-b-4 focus:placeholder:opacity-0 placeholder:font-ubuntu placeholder:text-[#0083e0] placeholder:opacity-50 placeholder:tracking-[0.3em] placeholder:font-bold md:placeholder:text-xl border-[#0082E0] bg-[#ACD0EA] bg-opacity-25 px-4 md:px-6"
    ...
    return(
        <div id='contact' className="md:space-y-20 space-y-10 pt-20 md:pt-24 mb-20">
            <div className="xl:mx-40 md:mx-24 mx-6 flex justify-end items-center text-[#0082E0] space-x-2">
                <h3 className="font-ubuntu font-bold uppercase md:text-3xl text-2xl">Contact</h3>
            ...
            <div className='xl:mx-40 md:mx-24 mx-6 md:flex justify-end'>
                <form className='space-y-4 flex flex-col md:w-4/5'  ref={form} onSubmit={sendEmail}>
                    ...
                    ? ""
                    :<>
                        <h4 className='font-ubuntu md:text-2xl text-xl tracking-widest font-semibold uppercase text-[#0082E0]'>{props.lng.contactme}</h4>
                        ...
                        <input type="submit" className='text-white cursor-pointer duration-200 ease-in-out hover:tracking-[0.1em] self-end font-ubuntu font-bold text-xl tracking-[0.3em] px-6 py-2 bg-[#0082E0] hover:bg-[#51b6fe]' value={props.lng.send} />
                    </>}
                        {send == "success"
                    ? <div className='py-2 h-1/2 flex justify-center items-center'>
                            <p className='text-[#0082E0] md:text-xl text-sm font-ubuntu font-bold tracking-wide'>{props.lng.send_success}</p>
                    </div>
                    : send == "failed"
